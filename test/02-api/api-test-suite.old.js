...
async function runAPITests() {
  ...
  await runTest('Screenshot Upload', async () => {
    const { response, data } = await apiRequest('/api/screenshot', {
      body: {
    ...
    if (!data.success && !data.id) {
      throw new Error('Screenshot response missing success or id');
    }
  });

  // 3. Console Logs
  await runTest('Console Logs Endpoint', async () => {
    const { response, data } = await apiRequest('/api/console-logs', {
  ...
  await runTest('Console Errors Endpoint', async () => {
    const { response, data } = await apiRequest('/api/console-errors', {
      body: { url: config.testData.urls.google }
    });
    if (!response || response.status !== 200) {
  ...
  await runTest('Network Logs Endpoint', async () => {
    const { response, data } = await apiRequest('/api/network-logs', {
      body: { url: config.testData.urls.google }
    });
    if (!response || response.status !== 200) {
  ...
  await runTest('Network Errors Endpoint', async () => {
    const { response, data } = await apiRequest('/api/network-errors', {
      body: { url: config.testData.urls.google }
    });
    if (!response || response.status !== 200) {
  ...
  await runTest('Lighthouse Audit', async () => {
    const { response, data } = await apiRequest('/api/lighthouse', {
      body: { 
  ...
  for (const auditType of auditTypes) {
    await runTest(`Lighthouse ${auditType} Audit`, async () => {
      const { response, data } = await apiRequest(`/api/lighthouse/${auditType}`, {
        body: { url: config.testData.urls.google }
      });
      if (!response || response.status !== 200) {
  ...
  await runTest('Authentication (Invalid Token)', async () => {
    const { response } = await fetch(config.getApiUrl('/api/lighthouse'), {
      method: 'POST',
      headers: {
    ...
    // Should still work for extension endpoints (they allow unauthenticated from extensions)
  ...
  await runTest('CORS Headers', async () => {
    const { response } = await apiRequest('/api/lighthouse', {
      body: { url: config.testData.urls.google }
    });
